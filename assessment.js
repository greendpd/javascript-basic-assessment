...
var secondHero=heroes.splice(heroes.indexOf('flash'),1)[0];
...
function removeItem(strArray,toRemove){
	return strArray.filter(function(cur,i,arr){
		return cur !=toRemove;
	})
}
...
function doubleTheFun(toDouble){
	if(Number(toDouble)){
		return 2*Number(toDouble);
	}
	return toDouble+toDouble;
	// if(toDouble instanceof String) return toDouble+toDouble;
	// if(toDouble instanceof Number) return toDouble*2;
}
